...
void push_stack_queue(tStack * s_s_a, tStack * s_s_b, int elem){
    push_stack(s_s_a, elem);
}

int pop_stack_queue(tStack * s_s_a, tStack * s_s_b){
    int elem;

    if(empty_stack(s_s_b)){
        if(empty_stack(s_s_a)){
            printf("Fila Vazia.\n");

            return -1;
        }else{
            while(!empty_stack(s_s_a)){
                elem = pop_stack(s_s_a);
                push_stack(s_s_b, elem);
            }
        }
    }

    elem = pop_stack(s_s_b);
    return elem;
}
